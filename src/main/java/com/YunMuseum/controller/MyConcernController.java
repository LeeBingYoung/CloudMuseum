...
public class MyConcernController {
    ...
    public String myConcern(Model model, @CookieValue("username") String username){
        int userid = clubService.selectClubUserID(username);
        //展示我关注的艺术品
        List<ArtWork> artWorkList = artWorkService.getArtWorkFollow(userid);
        model.addAttribute("ArtWork",artWorkList);
        //展示我关注的艺术专栏
        List<ArtCol> artColList = artColService.getArtColFollow(userid);
